  ...
  const saveCreatorUserTags = async (values) => {
    ...
    try {
      ...
      if (isAPISuccess(status)) {
        showSuccessModal('User tag settings updated successfully');
        fetchUserSettings();
      }
    } catch (error) {
      showErrorModal('Failed updating user tags settings', error.response?.data?.message || 'Something went wrong');
    }
